...
export class OilVisuals {
    ...
    private addCapsule(
        ...
        if (len < 0.5) {
            ...
            this.outline.add(disc);
            this.outline.add(this.circleRing(startX, startZ, radius, lineMat));
            return;
        }
        ...
        const pz = ux * radius;
        this.outline.add(
            new Mesh(this.buildCapsuleFill(startX, startZ, endX, endZ, px, pz, ux, uz, radius), fillMat),
        );
        for (const [x, z] of [
            [startX, startZ],
            [endX, endZ],
            ...
            this.outline.add(disc);
            this.outline.add(this.circleRing(x, z, radius, lineMat));
        }
        for (const [x0, z0, x1, z1] of [
            ...
            this.outline.add(new Line(geo, lineMat));
        }
    }

    private circleRing(x: number, z: number, radius: number, lineMat: LineBasicMaterial): Line {
        return new Line(
